  ...
  outputs = { self, nixpkgs, microvm, finix, flake-parts, import-tree, microbe-demo }:
    let
      ...
      microbePkg = pkgs.buildGoModule {
        ...
        # Section-1 man pages, generated by the just-built binary's own
        # hidden `gendocs man` command (internal/cmd/gendocs.go, wrapping
        # cobra/doc) -- same installShellFiles/installManPage pattern
        # nixpkgs uses for other Cobra-based CLIs (e.g. gh). No separate
        # build step or throwaway binary: it's the same microbe binary
        # generating docs for its own command tree.
        nativeBuildInputs = [ pkgs.installShellFiles ];
        postInstall = ''
          $out/bin/microbe gendocs man man1
      ...
      docs = import ./nix/docs.nix {
        inherit pkgs lib nixpkgs system microbePkg;
