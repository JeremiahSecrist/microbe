  ...
  outputs = { self, nixpkgs, microvm, finix, flake-parts, import-tree, microbe-demo }:
    let
      ...
      microbePkg = pkgs.buildGoModule {
        ...
        # Only ship the CLI and the guest agent binary it writes into
        # generated service configs (internal/nix/flakegen). Without this,
        # buildGoModule installs every `package main` under the module --
        # including cmd/gendocs, which is a build-time-only doc generator
        # (see postInstall below), never meant to reach $out/bin.
        subPackages = [ "." "./internal/nix/flakegen/agent" ];
        ...
        # Section-1 man pages: `go run ./cmd/gendocs` (cobra/doc's own
        # GenManTree, cobra's documented pattern -- a small external program
        # given the *cobra.Command, not a command wired into the CLI itself)
        # against the same vendored deps this derivation already fetched, no
        # network and no separate buildGoModule/vendorHash needed. Same
        # installShellFiles/installManPage pattern nixpkgs uses for other
        # Cobra-based CLIs (e.g. gh). gendocs is never installed to $out --
        # go run doesn't leave a binary behind, unlike building+deleting one.
